<template>
  <div class="cuckoo-input-container">
    ...
    <textarea v-show="shouldShowSpoilerTextInputArea" ref="spoilerTextArea"
      class="auto-size-text-area spoiler-text-area base-theme-bg-color" v-model="spoilerTextValue"
      :placeholder="$t($i18nTags.common.write_your_warning_here)" />

    <textarea ref="textArea" class="auto-size-text-area" v-model="textValue" @keydown.stop="onKeyDown"
      @keydown.ctrl.enter="onQuickSubmit" @input="onInput" @keydown.38="onMinisSelectedResultIndex"
      @keydown.40="onPlusSelectedResultIndex" @keydown.enter="onSelectedSearchResult" @click="onTextAreaClick"
      :placeholder="placeholder" />

    <div v-if="uploadProcesses.length" class="media-area" :class="{ 'single-media-area': uploadProcesses.length === 1 }">
      <div class="media-item" :key="index" v-for="(processInfo, index) in uploadProcesses">
        ...
        <div class="remove-icon-wrapper" @click="onRemoveMediaFileByIndex(index)">
          <svg height="24px" width="24px" viewBox="0 0 48 48">
            ...
            <path fill="#000"
              d="M24,4C12.9,4,4,12.9,4,24s8.9,20,20,20s20-9,20-20S35,4,24,4z M34,31.2L31.2,34L24,26.8L16.8,34L14,31.2l7.2-7.2L14,16.8l2.8-2.8l7.2,7.2l7.2-7.2l2.8,2.8L26.8,24L34,31.2z">
            </path>
          </svg>
        </div>
      </div>
    ...
    <mu-list v-if="shouldShowAccountSearchResultList" v-loading="isLoadingSearchResult"
      class="at-account-search-result-list dialog-theme-bg-color" :style="accountSearchResultListStyle">
      <mu-list-item avatar button :ripple="false" :key="index" @hover="currentSelectedResultIndex = index"
        @click.stop="onSelectedSearchResult" :class="{ 'active': currentSelectedResultIndex === index }"
