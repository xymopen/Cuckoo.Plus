<template>
  <mu-popover cover :open.sync="shouldOpen" :trigger="trigger">
    <mu-list textline="two-line">
      <mu-list-item button v-for="(visibilityType, index) in VisibilityTypeList"
        :class="{ 'selected-item': visibilityType === visibility }" :key="index"
        @click.stop="onChangeVisibility(visibilityType)">
        <mu-list-item-action>
        ...
        <mu-list-item-content>
          ...
          <mu-list-item-sub-title class="secondary-read-text-color">{{ $t(getVisibilityDescInfo(visibilityType).descTag)
