<template>
  <div class="full-reply-list-item" v-loading="isListItemLoading" @mouseover="onItemMouseOver" @mouseout="onItemMouseOut">
    ...
    <div class="content-area" ref="contentArea">
      ...
      <div class="content-header">
        <div class="reply-user-display-name">
          ...
          <span v-if="status.favourites_count > 0" class="reply-favorites-count"
        ...
        <div class="operation-area" ref="operationArea" :style="operationAreaStyle">
          <span v-show="!shouldOpenMoreOperationPopOver && !shouldShowMoreOperationTriggerBtn"
            class="reply-from-now secondary-read-text-color">{{ getFromNowTime() }}</span>
          <mu-button v-show="shouldOpenMoreOperationPopOver || shouldShowMoreOperationTriggerBtn" icon
            style="width: 16px; height: 16px" @click="onOpenMoreOperationPopOver">
      ...
      <div class="reply-action-list">
        ...
        <div class="plus-one-button secondary-theme-text-color" @click="onFavoriteButtonClick"
          :class="{ 'primary-theme-bg-color': status.favourited }">
          <a>+1</a>
        </div>

        <div class="reshare-button secondary-theme-text-color" @click="onReBlogButtonClick"
    ...
    <mu-popover cover placement="left-start" :open.sync="shouldOpenMoreOperationPopOver"
      ...
      <mu-list>
        ...
        <mu-list-item button v-if="currentUserAccount.id !== status.account.id" @click.stop="onMuteUser">
          <mu-list-item-title>{{ $t($i18nTags.statusCard.mute_user) }}</mu-list-item-title>
        </mu-list-item>
        <mu-list-item button v-if="currentUserAccount.id === status.account.id" @click.stop="onDeleteStatus">
