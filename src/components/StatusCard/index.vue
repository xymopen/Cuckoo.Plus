<template>
  <div class="status-card-container" @dragenter="onDragFileOver" ref="statusCardContainer">
    <mu-card class="status-card status-card-bg-color" v-loading="isCardLoading" v-drag-over="isFileDragOver"
      @cuckooDragOver="onDragFileOver" @cuckooDragleave="isFileDragOver = false" @cuckooDrop="onDropFile">

      <card-header :status="status" @deleteStatus="isCardLoading = true" @muteStatus="onMuteStatus"
        @muteUser="onMuteUser" />
      ...
      <div class="spoiler-text-area primary-read-text-color" v-if="status.spoiler_text">
        ...
        <mu-button flat small class="secondary-theme-text-color" :style="{ minWidth: 'unset' }"
          ...
          {{ $t(shouldShowContentWhileSpoilerExists ? $i18nTags.statusCard.hide_content :
            $i18nTags.statusCard.show_content) }}
        </mu-button>
      </div>

      <mu-card-text v-if="!status.reblog && status.content"
      ...
      <div class="reply-area-full">
        <div class="full-reply-list" ref="replyListContainer">
          <full-reply-list-item v-for="replierStatus in descendantStatusList" @muteStatus="onMuteStatus"
            @muteUser="onMuteUser" :key="replierStatus.id" :status="replierStatus"
            @reply="onReplyToStatus(replierStatus)" />
        </div>
      </div>
      ...
      <mu-card-actions class="card-action-area">
        <simple-action-bar v-show="!shouldShowFullReplyActionArea" :status="status" @reply="onReplyToStatus(status)" />

        <full-action-bar v-if="isOAuthUser && shouldShowFullReplyActionArea" :currentReplyToStatus="currentReplyToStatus"
          :descendantStatusList="descendantStatusList" :droppedFiles="droppedFiles"
          :replySpoilerText.sync="replySpoilerText" :status="status" :value.sync="replyInputValue"
          @hide="hideFullReplyActionArea" @loadingStart="isCardLoading = true" @loadingEnd="isCardLoading = false"
