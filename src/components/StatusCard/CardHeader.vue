<template>
  <mu-card-header class="mu-card-header" ref="cardHeader" @mouseover="shouldShowHeaderActionButtonGroup = true"
    ...
    <div class="right-area" ref="rightArea" v-if="isOAuthUser">
      <span v-show="!shouldOpenMoreOperationPopOver && !shouldShowHeaderActionButtonGroup"
        class="status-from-now secondary-read-text-color">{{ getFromNowTime(status.created_at) }}</span>
    ...
    <mu-popover v-if="isOAuthUser" cover placement="left-start" :open.sync="shouldOpenMoreOperationPopOver"
      ...
      <mu-list>
        ...
        <mu-list-item button v-if="currentUserAccount.id !== status.account.id" @click.stop="onMuteUserByOperateList">
          <mu-list-item-title>{{ $t($i18nTags.statusCard.mute_user) }}</mu-list-item-title>
        </mu-list-item>
        <mu-list-item button v-if="currentUserAccount.id === status.account.id" @click.stop="onDeleteStatusByOperateList">
