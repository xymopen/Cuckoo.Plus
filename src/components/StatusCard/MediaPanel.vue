  ...
  <div class="media-panel-container" v-if="combinedMediaList.length > 0">
    <div class="media-area" ref="mediaArea" :style="mediaAreaScrollStyle"
      ...
      <place-holder-media-item class="media-item" v-for="(media, index) in combinedMediaList" :key="index"
        @click.native="onMediaItemClick(index)" :holderStyle="getMediaSizeStyle(index)" :url="media.url"
        :mediaType="media.type" :shouldShowSensitiveCover.sync="shouldShowSensitiveCover" />
    ...
    <mu-dialog class="light-box" transition="fade" ref="lightBox" @click.native.stop="onLightBoxClick"
      ...
      <mu-carousel :cycle="false" :active="lightBoxActiveIndex" transition="fade"
        ...
        <mu-carousel-item v-for="(mediaInfo, index) in combinedMediaList" :key="index">
          <div class="light-box-item" @click.stop="onLightBoxMediaItemClick">
            ...
            <video v-if="mediaInfo.type === mediaTypes.GIFV || mediaInfo.type === mediaTypes.VIDEO" controls
              :loop="mediaInfo.type === mediaTypes.GIFV" :src="mediaInfo.url" />
          </div>
        </mu-carousel-item>
      </mu-carousel>
    </mu-dialog>
