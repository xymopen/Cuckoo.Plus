<template>
  <div class="placeholder-media-item-container" :style="placeHolderItemStyle">
    ...
    <img v-show="isMediaLoaded" v-if="(mediaType === mediaTypes.IMAGE)" :class="[showSensitiveCover && 'sensitive-hide']"
      :src="url" @load="onMediaContentLoaded" @error="onMediaContentLoadFailed" />

    <div class="gifv-container" v-show="isMediaLoaded"
      v-if="mediaType === mediaTypes.GIFV || mediaType === mediaTypes.VIDEO">
      <video width="100%" controls :loop="mediaType === mediaTypes.GIFV" :class="[showSensitiveCover && 'sensitive-hide']"
        :src="url" @loadstart="onMediaContentLoaded" @error="onMediaContentLoadFailed" />
    </div>

    <mu-button class="hide-sensitive-btn" v-show="!showSensitiveCover && isMediaLoaded"
      @click.stop="onShowSensitiveCover">
