    ...
    <mu-card v-loading="isLoading">
      <mu-card-actions class="setting-card">
        ...
        <div class="foot-note secondary-read-text-color">
          ...
          /
          <span
            @click="shouldOpenThemeColorSetExportDialog = true">{{ $t($i18nTags.settings.export_theme_color_set) }}</span>
          /
        ...
        <div class="setting-row select-row">
          <span
            class="setting-label primary-read-text-color">{{ $t($i18nTags.settings.maximum_number_of_columns_in_multi_line_mode) }}</span>
        ...
        <div class="setting-row select-row">
          ...
          <mu-select class="setting-select" v-model="postPrivacy">
            <mu-option v-for="(visibilityInfo, index) in postPrivacyOptions" :key="index" :label="visibilityInfo.label"
